...
Обязательное условие — id должен быть первичным или уникальным ключом (должен быть индекс). Лучше числом, но можно и текст.
Запрос выполняем в бесконечном цикле. На первой итерации для числового типа вместо `:id` подставляем `0` (или другое значение, меньше минимального в таблице);  для текстового типа вместо `:id` подставляем пустое зачение (`''`). На второй и последующих итерациях вместо `:id` подставляем id из последней записи последнего результата запроса (это значение будет являться максимальным в рамках результата запроса). На каждой итерации цикла проверяем: если количество возвращёных записей меньше, чем указано в LIMIT, то цикл прерываем. Функция [`use_parallel.sql`](functions/use_parallel.sql).

**Вариант 2a**

Способ, когда нужно заранее вычислять диапазоны id. Считаю этот способ устаревшим, т.к. он работает медленнее и выглядит сложнее.
