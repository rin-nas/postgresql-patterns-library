...
Для сохранения истории изменения записей в таблице-журнале используется подход, который имеет следующие преимущества:
...
DDL команды обычно логируются в общем файловом журнале.
В огромном общем файловом журнале полно SELECT и DML команд, отыскать там DDL непросто.
Файловые журналы обычно хранятся 1-2 недели и потом автоматически удаляются.

Логирование DDL команд в отдельной таблице БД позволяет:
1. Усилить безопасность БД:
   * сохранять легальные изменения схемы БД (DBA)
   * выявить нелегальные изменения схемы БД: прямое выполнения DDL команд мимо миграторов БД; SQL injection
1. Иметь возможность получить и предоставить данные по запросу коллег при возникновении инцидентов, включая нестабильность работы БД. При этом будет возможность получить точную дату-время начала и окончания, а так же длительность выполнения DDL запросов.
1. Знать дату-время создания новых объектов БД, чтобы потом автоматически их удалять из схемы `migration` и `unused` через 1 месяц после создания 

Готовое решение: [`DDL_log_to_table.sql`](dba/DDL_log_to_table.sql)
