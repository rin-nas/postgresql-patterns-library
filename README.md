...
```sql
CREATE DOMAIN email AS varchar(320) CHECK(VALUE = trim(VALUE) AND VALUE LIKE '%_@_%._%' AND is_email(VALUE));
```
...
**Медленные запросы** это те, которые могли бы работать в разы быстрее, если их переписать по-другому и/или добавить индексы. Для принятия решения об оптимизации нужно учитывать: 
* длительность выполнения одного запроса
* кол-во выполнения запросов за единицу времени
* объём передаваемых и возвращаемых данных, их структуру
