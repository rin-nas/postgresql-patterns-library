...
**[Модификация пользовательских данных (DML)](#Модификация-пользовательских-данных-DML)**
   ...
   1. [Как обновить или удалить десятки тысяч записей в таблице не блокируя все записи и не нагружая БД?](#Как-обновить-или-удалить-десятки-тысяч-записей-в-таблице-не-блокируя-все-записи-и-не-нагружая-БД)
...
1. обработки большого количества записей в админках
...
from (select count(*) as locked_deleted from d1) as r1,
     ...
     (select batch_limit from options) AS o;
```
