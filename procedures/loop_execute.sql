...
Предназначена для миграций данных в БД.
Принцип работы -- выполняет CTE DML запрос в цикле. 
После завершения каждого цикла изменения фиксируются (либо откатываются для целей тестирования, это настраивается).
Процедура автоматически адаптируется под нагрузку на БД: если длительность выполнения DML запроса на итерации цикла превышает 
установленное ограничение времени блокировки строк, то количество модифицируемых записей для следующей итерации цикла уменьшается в 2 раза. И наоборот.
Внутри процедуры никакие курсоры не создаются.
