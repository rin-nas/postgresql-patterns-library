...
sys    0m5.663s
```
Многопоточный `zstd` работает существенно быстрее многопоточного `gzip`.
...
sys    0m0.725s
```
Встроенная однопоточная компрессия (`gzip`) работает относительно медленно. 
...
sys    0m0.848s
```
Встроенная многопоточная компрессия (`gzip`) работает быстрее однопоточной.
