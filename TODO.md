...
Предполагаемое решение — измерять скорость выполнения каждого запроса (SELECT или DML). Если оно превышает 1 секунду, значит ресурсов БД нехватает и после выполнения запроса приложении нужно замедлить, т.е. "поспать" некоторое время. Такой трюк даст "продохнуть" БД и адаптироваться под нагрузку. Длительность сна можно высчитывать по формуле `greatest(ln(exec_time), 0)`. См. распределение значений запросом `select exec_time, greatest(ln(exec_time), 0) as sleep_time from generate_series(0.1, 20, 0.1) as exec_time;`
