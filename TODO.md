...
Имеется таблица
```sql
create table v3_person_password_log
(
    id                   integer generated by default as identity
        primary key,
    person_id            integer                                   not null
        references v3_person
            on update cascade on delete cascade,
    password_hash        varchar(60)                               not null
        constraint v3_person_password_log_password_hash_check
...
comment on table v3_person_password_log is 'История изменений паролей пользователей';
comment on column v3_person_password_log.id is 'ID записи';
comment on column v3_person_password_log.person_id is 'ID персоны';
comment on column v3_person_password_log.password_hash is 'Соль и хеш от пароля в формате bcrypt';
comment on column v3_person_password_log.created_at is 'Дата-время создания';
comment on column v3_person_password_log.http_request_headers is 'Заголовки HTTP запроса в формате ключ-значение';
comment on column v3_person_password_log.request_remote_addr is 'IP адрес запроса';
comment on column v3_person_password_log.session_id is 'ID сессии, значение из session.sid';
comment on column v3_person_password_log.is_auto_generated is 'Признак генерации пароля системой';
```
...
from public.v3_person_password_log as t
...
```sql
update public.v3_person_password_log
