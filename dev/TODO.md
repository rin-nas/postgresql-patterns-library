...
```
  ...
  1. https://postgrespro.ru/docs/postgresql/current/logical-replication
...
Имеется очень быстрый способ "огнетушителя" избавиться от bloat в таблицах, где кол-во записей < 10000. Проверил на прод БД. Всё работает отлично.

```sql
DO $$
    BEGIN
        SET LOCAL lock_timeout TO '3s'; -- Максимальная длительность ожидания получения блокировки объекта СУБД (в данном случае таблицы) SQL запросами в этой транзакции. Если будет превышено, то транзакция откатится.
...
-- https://postgrespro.ru/docs/postgresql/current/storage-toast
...
--https://www.postgrespro.ru/docs/postgresql/current/plpgsql-trigger
...
--https://www.postgrespro.ru/docs/postgresql/current/sql-createtrigger
