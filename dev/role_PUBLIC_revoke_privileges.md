...
Существует групповая псевдороль [`PUBLIC`](https://postgrespro.ru/docs/postgresql/current/ddl-priv), которая неявно включает в себя все остальные роли.
Это означает, что все роли по умолчанию будут иметь привилегии, наследуемые от `PUBLIC`.
Если выдать какие-либо привилегии роли `PUBLIC`, эти привилегии получат вообще все роли.

Роль `PUBLIC` по умолчанию имеет следующие привилегии:

**для всех баз данных:**
* `CONNECT` — это означает что любая созданная роль сможет подключаться к базам данных (не путайте с привилегией `LOGIN`);
...
**для всех функций:**
* `EXECUTE` (выполнение) — любая роль может выполнять любую функцию. 
  Ещё нужны права `USAGE` на ту схему, в которой функция находится, и права к объектам к которым обращается функция.

Для увеличения безопасности у роли `PUBLIC` отбирают некоторые привилегии, чтобы отнять их у всех пользователей.

## Решение — отзываем у роли PUBLIC все привилегии
