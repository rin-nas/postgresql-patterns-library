...
1. Описания объектов БД (`COMMENT ON ...`)
   1. Добавить проверку наличия описания для объектов БД: схемы, представления, функции, процедуры, триггеры, типы, домены. 
   ...
   1. В одной БД описания всех схем должны быть уникальным.
   1. В одной схеме описания всех таблиц должны быть уникальным. 
   1. В одной таблице описания всех колонок должны быть уникальным.
1. Названия объектов БД
   1. 🚨 Названия объектов БД (схемы, таблицы, представления, колонки, функции, процедуры, триггеры, ограничения, типы, домены) 
      должны содержать только английские буквы, цифры, дефис, тире: `select 'col_name5' ~ '^[a-z][a-z\d_\-]*[a-z\d]$';`.
