...
1. Архитектурные доработки (нужно добавить настройки в конфиг)
   ...
   1. Добавить в таблицу `schema_validate_config` колонку `columns_ignore` (`columns_ignore_regexp` не нужна, а то сложно).
      Добавить обработку этой опций в каждую проверку. 
      Тип колонки должен быть [составным типом](https://postgrespro.ru/docs/postgresql/14/rowtypes): `row(table_name regclass, column_name text)`.
