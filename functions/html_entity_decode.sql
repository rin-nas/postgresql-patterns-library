create or replace function html_entity_decode(s text)
returns text
language plpgsql
as $func$
    declare
        ...
        map constant jsonb not null default $${
  ...
  "&zwnj;": { "codepoints": [8204], "characters": "\u200C" }
        ...
        r record;
        esc constant char(1) not null default e'\u0001';
    begin
        for r in
            select distinct
                m[1] as name
        ...
        for r in
            select distinct
                m[1] as name,
                m[2]::int as codepoint
            from regexp_matches(s, '(&#([0-9]{1,10});)', 'g') as m
        loop
            s := replace(s, r.name, esc || chr(r.codepoint));
        end loop;

        for r in
            select distinct
                m[1] as name,
                ('x' || repeat('0', 8 - octet_length(m[2])) || m[2])::bit(32)::int as codepoint
            from regexp_matches(s, '(&#x([\da-fA-F]{1,8});)', 'g') as m
        loop
            s := replace(s, r.name, esc || chr(r.codepoint));
        end loop;

        return replace(s, esc, '');
    end;
$func$;
