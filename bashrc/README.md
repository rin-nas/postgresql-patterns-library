...
1. хост
1. IP адреса (несколько IP разделяются пробелом)
1. путь и каталог
...
## Как сделать так, чтобы при взятии `root` (`sudo -i`) профиль брался от исходного аутентифицированого пользователя?

На Ubuntu 25 это не работает, т.к. под суперпользователем команда `who -m` возвращает пустую строку вместо залогиненного пользователя.

Отредактируйте файл `/root/.bash_profile` или `/root/.profile` (в зависимости от разновидности Linux).
...
* [Удобное цветное приглашение командной строки в `psql`](../dev/psqlrc.advanced)
