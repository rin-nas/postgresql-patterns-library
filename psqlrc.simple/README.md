...
Желательно использовать учётную запись суперпользователя (например, `postgres`), иначе может работать не полностью (из-за нехватки прав доступа к объектам СУБД).
...
1. названия серверов, откуда принимаются WAL файлы и куда передаются
