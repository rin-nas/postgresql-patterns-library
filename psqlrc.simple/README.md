...
Необходимо использовать учётную запись суперпользователя (например, `postgres`), иначе может работать не полностью (из-за нехватки прав доступа к объектам СУБД).
...
1. название и версия сервера: основная (мажорная) и дополнительная (минорная)
1. пользователь
1. хост
1. порт
1. база данных
