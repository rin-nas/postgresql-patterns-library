...
: <<'COMMENT'
  Проблема.
    Представьте схему кластера СУБД из 4-х узлов, который работает под управлением Patroni.
    В основном ЦОДе мастер и реплика, они имеют общую сетевую папку для резервных копий и архивации WAL файлов.
    В резервном ЦОДе две реплики, они имеют общую сетевую папку для резервных копий и архивации WAL файлов.
    В каждом ЦОДе сетевые папки разные. Все 3 реплики забирают изменения с мастера, каскадных реплик нет.
    Чтобы в резервном ЦОДе работало архивирование WAL файлов на реликах, необходимо установить параметр archive_mode=always.
    Но с разных экземпляров СУБД возникает конкурентная запись WAL файлов в общие сетевые папки.
  Решение.
    Этот скрипт исключает повторную запись одних тех же WAL файлов (экономя ресурсы сети и диска) и риск их некорректной записи.
    ...
    Для экономии места на диске WAL файлы сжимаются через zstd с низким приоритетом (для экономии ресурсов CPU).
    Уровень компрессии высчитывается динамически: чем больше WAL файлов в очереди на архивацию, тем меньше степень сжатия (но больше скорость сжатия и размер сжатого файла).
COMMENT
...
# для отладки и просмотра кол-ва жёстких ссылок на файл используйте утилиту stat (не используйте ls, она берёт информацию из кеша)
...
# как быстро будет уменьшаться уровень компрессии, начиная с максимального
ZSTD_LEVEL_STEP=3
# максимальный уровень компрессии (не ставьте большим, это приводит к большому потреблению CPU и памяти, а экономия на размере файла несущественная!)
ZSTD_MAX_LEVEL=9
