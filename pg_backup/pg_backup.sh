...
if test "${1:-}" = "ExecCondition"; then
  ...
  exit
# проверяем восстанавливаемость PostgreSQL из резервной копии
  ...
  BACKUP_FILE=$(find $BACKUP_DIR -maxdepth 2 -type f \( -name "*.pg_basebackup.tar.*" -o -path "*.pg_basebackup/base.tar.*" \) \
  ...
  if test -z "$BACKUP_BASE_DIR"; then
    ...
    echo "Копируем '$BACKUP_BASE_DIR/backup_manifest' в '$PG_DATA_TEST_DIR'"
    cp $BACKUP_BASE_DIR/backup_manifest $PG_DATA_TEST_DIR
    if test -f "$BACKUP_BASE_DIR/pg_wal.tar"; then
      echo "Распаковываем архив '$BACKUP_BASE_DIR/pg_wal.tar' в '$PG_DATA_TEST_DIR/pg_wal'"
      tar -xf $BACKUP_BASE_DIR/pg_wal.tar --directory=$PG_DATA_TEST_DIR/pg_wal
    elif test -f "$BACKUP_BASE_DIR/pg_wal.tar.$BACKUP_FILE_EXT"; then
      echo "Распаковываем архив '$BACKUP_BASE_DIR/pg_wal.tar.$BACKUP_FILE_EXT' в '$PG_DATA_TEST_DIR/pg_wal'"
      tar -xf $BACKUP_BASE_DIR/pg_wal.tar.$BACKUP_FILE_EXT --use-compress-program="$COMPRESS_PROGRAM" --directory=$PG_DATA_TEST_DIR/pg_wal
    else
      echoerr "В папке '$BACKUP_BASE_DIR' файлы 'pg_wal.tar' или 'pg_wal.tar.$BACKUP_FILE_EXT' не найдены" && exit 1
    fi
  fi
...
TIME_START=$(date +%s) # время в Unixtime
BASE_NAME=${BACKUP_DIR}/$(date +%Y-%m-%d.%H%M%S).$(hostname)
...
if test "$IS_BACKUP_WAL" = "f"; then
  ...
  ${PG_BIN_DIR}/pg_basebackup --username=${PG_USERNAME} --no-password --wal-method=none --checkpoint=fast --format=tar \
                              --pgdata=- | zstd -q -T${ZSTD_THREADS} -5 -o ${BASE_NAME}.pg_basebackup.tar.zst
else
