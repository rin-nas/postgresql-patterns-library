...
Необходимо использовать учётную запись суперпользователя (например, `postgres`), иначе может работать не полностью или не корректно (из-за нехватки прав доступа к объектам СУБД).
...
1. роль сервера: основной (мастер) `primary` или резервный (реплика) `standby`
1. пользователь@хост:порт/база_данных.схема
